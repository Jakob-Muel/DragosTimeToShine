...
const DRAGON_SOURCE := "res://assets/art/flight/flight_dragon_source.png"
const DRAGON_OUTPUT := "res://assets/art/flight/flight_dragon.png"
const SPIKES_SOURCE := "res://assets/art/flight/rock_spikes.png"
const SPIKES_OUTPUT := "res://assets/art/flight/rock_spikes_game.png"
const PADDING := 12
...
func _init() -> void:
	...
	if not _crop_and_save(dragon, DRAGON_OUTPUT, 420):
		quit(1)
		return

	var spikes := Image.load_from_file(SPIKES_SOURCE)
	if spikes.is_empty():
		push_error("Could not load " + SPIKES_SOURCE)
		quit(1)
		return
	spikes.convert(Image.FORMAT_RGBA8)
	if not _crop_and_save(spikes, SPIKES_OUTPUT, 180):
		quit(1)
		return

	print("Prepared flight assets.")
	quit()
...
func _crop_and_save(image: Image, path: String, target_width: int) -> bool:
