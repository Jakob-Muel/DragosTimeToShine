...
const SCREENS := ["main", "habitat", "shop", "dragons_elements", "groom", "flight_training_water", "flame_shooter", "flight_contest"]
...
func _capture() -> void:
	...
	for screen: String in review_screens:
		...
		assert(viewport.get_texture().get_image().save_png("res://docs/screenshots/unified/" + screen + ("_short" if short_phone else "") + ".png") == OK)
...
func _overview() -> void:
	...
	var title := WidgetFactory.label("DRAGO’S · EIN DRACHE, ALLE ANSICHTEN", 36, UiTokens.INK, HORIZONTAL_ALIGNMENT_LEFT, UiTokens.FONT_HEAVY)
	title.position = Vector2(30, 15)
	title.size = Vector2(1540, 55)
	board.add_child(title)
	var screens := ["main", "habitat", "flight_training_water", "flame_shooter"]
	var captions := ["EINE GEMEINSAME INSEL", "PROZEDURALE DRACHEN", "FLUGANSICHT", "KLEINERE FIGUREN"]
	for index in screens.size():
		...
		var art := ImageTexture.create_from_image(Image.load_from_file("res://docs/screenshots/unified/" + screens[index] + ".png"))
		var screenshot := WidgetFactory.texture_rect(art, Rect2(2, 2, 360, 782.5), TextureRect.STRETCH_SCALE)
		panel.add_child(screenshot)
	await process_frame
	await RenderingServer.frame_post_draw
	assert(board.get_texture().get_image().save_png("res://docs/screenshots/unified/overview.png") == OK)
